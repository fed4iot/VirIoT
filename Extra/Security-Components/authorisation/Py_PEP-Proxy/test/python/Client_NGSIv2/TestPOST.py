...
if __name__ == '__main__':
    ...
    policy_action = cfg.get("GENERAL", "policyPOST_action")
    policy_device = cfg.get("GENERAL", "policyPOST_device")
    policy_resource = cfg.get("GENERAL", "policyPOST_resource")
    ...
    if(keyrock_protocol.upper()=="http".upper() or keyrock_protocol.upper()=="https".upper()):
        ...
        if(status==201):
            ...
            if(status==200):
                ...
                bodyPEP=json.dumps(
                    {                
                        "id": "urn:ngsi-ld:sector:Sector:5",
                        "type": "sector",
                        "name": { "type": "Text", "value": "Sector 5", "metadata": {} }
                    }
                ).encode()


                print("\n******* Sending NGSIv2 query to MDR through PEP_PROXY... *******")
                ...
                if(pep_protocol.upper()=="http".upper()):
                    ...
                    conn = http.client.HTTPSConnection(pep_host,pep_port,
                                                    context=gcontext)
                conn.request(policy_action, policy_resource, bodyPEP, headers)
                response = conn.getresponse()
