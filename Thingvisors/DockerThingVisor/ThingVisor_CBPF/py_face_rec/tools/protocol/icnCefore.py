        ...
        if info.is_succeeded and (info.name == INT_CONTENT) and (info.chunk_num == 0):
            ...
            if final_chunk_num != 0:
                ...
                for i in range (final_chunk_num):
                    ...
                    print ("[icnGetContent] send Interst: {}".format(INT_CONTENT + "/" + str(i+1)))
                ...
                for i in range(final_chunk_num):
                    ...
                    if info.is_succeeded and (info.name == INT_CONTENT) and (info.chunk_num == i+1):
                        ...
                        print ("[icnGetContent] received segment ID: {}".format(str(i+1)))
            ...
            endTime = time.time()
            qos = getQoS(startTime, startTime2, endTime, receive_data)
            break
...
def updateFib(fib):
    ...
    if "protocol" not in fib:
        fib.update({'protocol': "udp"})

    CMD = ["cefroute $0 $1 $2 $3", fib['method'], fib['name'], fib['protocol'], fib['src']]
