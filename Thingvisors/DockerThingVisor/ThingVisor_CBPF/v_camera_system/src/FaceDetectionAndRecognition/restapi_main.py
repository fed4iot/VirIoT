...
def uploads():
    ...
    """
        ...
        src_img = base64.b64decode(data['content']['value'].encode('utf-8'))
        file_name = data['file name']['value']
...
def return_result():
    if request.method == 'GET':
        ...
        JSON_FILE_SEND_PATH = config.json_file_dir + config.send_json_file
        
        #change file name
        if os.path.exists(JSON_FILE_PATH) is True:
            print ("face is detected!")
            shutil.copy(JSON_FILE_PATH, JSON_FILE_SEND_PATH, follow_symlinks=True)

            with open(JSON_FILE_SEND_PATH, 'r') as f:
                send_json = json.load(f)

            #os.remove(JSON_FILE_SEND_PATH)
