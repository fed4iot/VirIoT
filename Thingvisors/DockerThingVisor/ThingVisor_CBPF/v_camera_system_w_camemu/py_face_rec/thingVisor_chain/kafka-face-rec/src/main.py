...
from service import face_find
#######

SETTING = "./config/nodeSetting.json"
#SERVICEMAP = "./config/serviceMap.json"

REQ_IMG = "./service/request.jpg"

if os.path.exists(REQ_IMG) is True:
    os.remove(REQ_IMG)
...
def actuateFromController(CONTROL, TOPIC_IN):
    ...
    for message in controller:
        ...
        if control_message['cmd-value'] == "start":
            print ("receive start command")

            req_img = control_message['cmd-params']['content']['value']
            _req_img = base64.b64decode(req_img.encode('utf-8'))
            with open(REQ_IMG, 'wb') as f:
                f.write(_req_img)

        elif control_message['cmd-value'] == "close":
            print ("receive close commad")

            if os.path.exists(REQ_IMG) is True:
                os.remove(REQ_IMG)
                print ("request image is deleted")
            else:
                pass
